...
if __name__=="__main__":
    """Main function part"""
    print("Please enter the folder (video file) address you want to convert (shortcut are supported)：",end='')
    filepath = input()
    if filepath[-1]=='\"' and filepath[0]=='\"':
        filepath = filepath[1:-1]
        # used to solve path's problem if there has "" in the given path
    ...
    for a2 in videolist:
        if len(a2.split('.')) == 2 and a2.split('.')[1] in ["mp4","ogv","webm","mov","avi"]:
            print("--" + a2)
    time.sleep(3)
    print("Please enter output format, mp3(lightweight) and wav(lossless) are supported ：",end='')
    outputform = input()
